# -*- coding: utf-8 -*-
# Script to generate the flat Workbench script forcing SpaceClaim Geometry
import os

def create_wb_automation_script_force_spaceclaim():
    script_path = r"C:\Users\JULIAN\JunoWorkspace\projects\DIPAS\code\wb_run_spaceclaim_update.py"
    
    content = """# -*- coding: utf-8 -*-
# Flat script executing inside ANSYS Workbench to update SpaceClaim geometry and Meshing (Forced SpaceClaim)
import os

# 1. Buscamos dinamicamente el sistema de Fluent en el proyecto
system = None
...
if not system:
    print("Error: No se encontro ningun sistema de Fluent en el proyecto.")
else:
    print(">> Sistema detectado en Workbench: " + system.Name + " (" + system.DisplayText + ")")
    
    # 2. Abrimos SpaceClaim forzandolo con IsSpaceClaimGeometry=True
    geometry_container = system.GetContainer(ComponentName="Geometry")
    print(">> Abriendo SpaceClaim en segundo plano (forzado)...")
    geometry_container.Edit(IsSpaceClaimGeometry=True, Interactive=False)
    
    # 3. Definimos las instrucciones en Python para SpaceClaim
    spaceclaim_cmd = \"\"\"
# Python Script para SpaceClaim - Version 7
import clr
...
\"\"\"
    ...
    print(">> Enviando comandos de automatizacion de geometria a SpaceClaim...")
    geometry_container.SendCommand(Language="Python", Command=spaceclaim_cmd)
    
    # 4. Actualizamos el componente de malla (Mesh) en Workbench
    print(">> Actualizando celda de Malla en Workbench...")
    mesh_component = system.GetComponent(Name="Mesh")
    mesh_component.Update(AllDependencies=True)
    
    print(">> [EXITO] Geometria y Malla actualizadas en Workbench.")
"""
    
    with open(script_path, "w") as f:
        f.write(content)
    print("Script de integracion Workbench-SpaceClaim con IsSpaceClaimGeometry=True creado.")

if __name__ == "__main__":
    create_wb_automation_script_force_spaceclaim()
