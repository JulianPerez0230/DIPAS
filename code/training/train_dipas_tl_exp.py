# -*- coding: utf-8 -*-
"""
Experimento 3: Transfer Learning + Datos Experimentales (UIUC).
Estructura de entrenamiento en 4 etapas:
Fase A: Pre-entrenamiento geométrico incondicional con 10.000 muestras de UniFoil.
Fase B: Adaptación geométrica incondicional con 1.620 muestras experimentales de UIUC (data/uiuc_cst_dataset.csv).
Fase C: Fine-tuning condicional con 10.000 muestras de XFOIL (data/dataset.csv).
Fase D: Fine-tuning/ajuste final con ~3.000 muestras de Fluent (data/dataset_cfd.csv).
"""
...
def main():
    unifoil_path = SCRIPT_DIR.parent / "data" / "unifoil_cst_dataset.csv"
    uiuc_path = SCRIPT_DIR.parent / "data" / "uiuc_cst_dataset.csv"
    xfoil_path = SCRIPT_DIR.parent / "data" / "dataset.csv"
    cfd_path = SCRIPT_DIR.parent / "data" / "dataset_cfd.csv"
    output_dir = SCRIPT_DIR.parent / "outputs"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    model_save_path = output_dir / "dipas_tl_exp_model.pth"
    loss_plot_path = output_dir / "dipas_tl_exp_losses.png"
    recon_plot_path = output_dir / "dipas_tl_exp_evaluation_extremes.png"
    
    if not all(p.exists() for p in (unifoil_path, uiuc_path, xfoil_path, cfd_path)):
        print("❌ Faltan datasets. Asegúrate de tener unifoil_cst_dataset.csv, uiuc_cst_dataset.csv, dataset.csv y dataset_cfd.csv.")
        sys.exit(1)
        
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f">> Iniciando Experimento 3 (TL + UIUC) en {device}...")
    
    # -------------------------------------------------------------
    # FASE A: Pre-entrenamiento con UniFoil (Incondicional)
    # -------------------------------------------------------------
    print("\n--- Fase A: Pre-entrenamiento con UniFoil (10k) ---")
    batch_size = 64
    epochs_unifoil = 80
    lr_unifoil = 0.001
    ...
    optimizer = optim.Adam(model.parameters(), lr=lr_unifoil)
    
    for epoch in range(1, epochs_unifoil + 1):
        model.train()
        epoch_loss = 0
        for batch_x, _ in unifoil_loader:
            batch_x = batch_x.to(device)
            batch_c = torch.zeros((batch_x.size(0), 4), dtype=torch.float32).to(device)
        ...
        epoch_loss /= len(unifoil_loader.dataset)
        if epoch % 20 == 0 or epoch == 1:
            print(f"  Epoch {epoch:02d}/{epochs_unifoil:02d} | UniFoil Loss: {epoch_loss:.5f}")
            
    print(">> Fase A completada.")
    
    # -------------------------------------------------------------
    # FASE B: Adaptación con UIUC (Incondicional, Re Bajo Experimental)
    # -------------------------------------------------------------
    print("\n--- Fase B: Adaptación con UIUC (1.620 perfiles) ---")
    epochs_uiuc = 50
    lr_uiuc = 0.0005
    
    uiuc_dataset = AirfoilCSTDataset(str(uiuc_path), use_conditions=False)
    uiuc_loader = DataLoader(uiuc_dataset, batch_size=batch_size, shuffle=True, drop_last=True)
    
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr_uiuc
        
    for epoch in range(1, epochs_uiuc + 1):
        model.train()
        epoch_loss = 0
        for batch_x, _ in uiuc_loader:
            batch_x = batch_x.to(device)
            batch_c = torch.zeros((batch_x.size(0), 4), dtype=torch.float32).to(device)
        ...
        epoch_loss /= len(uiuc_loader.dataset)
        if epoch % 15 == 0 or epoch == 1:
            print(f"  Epoch {epoch:02d}/{epochs_uiuc:02d} | UIUC Loss: {epoch_loss:.5f}")
            
    print(">> Fase B completada.")
    
    # -------------------------------------------------------------
    # FASE C: Fine-tuning condicional con XFOIL (10k)
    # -------------------------------------------------------------
    print("\n--- Fase C: Fine-tuning Condicional con XFOIL (10k) ---")
    epochs_xfoil = 60
    lr_xfoil = 0.0005
    ...
    for epoch in range(1, epochs_xfoil + 1):
        ...
        if epoch % 20 == 0 or epoch == 1:
            print(f"  Epoch {epoch:02d}/{epochs_xfoil:02d} | XFOIL Loss: {epoch_loss:.5f}")
            
    print(">> Fase C completada.")
    
    # -------------------------------------------------------------
    # FASE D: Fine-tuning de alta fidelidad con Fluent (3k)
    # -------------------------------------------------------------
    print("\n--- Fase D: Fine-tuning final con Fluent ---")
    epochs_cfd = 60
    lr_cfd = 0.0001
    ...
    print(f"\n>> Modelo final DIPAS TL+Exp guardado en: {model_save_path}")
    
    # Graficar curva de pérdidas
    plt.figure(figsize=(8, 4))
    plt.plot(train_losses, label="Train Loss (CFD)")
    plt.plot(val_losses, '--', label="Val Loss (CFD)")
    plt.title("Curva de Pérdidas de Ajuste Fino CFD - DIPAS TL+Exp (Exp 3)")
    plt.xlabel("Épocas")
    plt.ylabel("Pérdida")
    plt.legend()
    plt.grid(True)
    ...
    print("==========================================================================")
    print("                      DIPAS TL+Exp (Exp 3): Metricas de Validacion        ")
    print("==========================================================================")
    ...
    for idx, (label, name) in enumerate(targets.items()):
        ...
        ax.plot(x_pts, y_up_rec, 'r--', label="DIPAS TL+Exp (IA)", linewidth=1.5)
    ...
    plt.close()
    print("\n>> Experimento 3 finalizado con éxito.")
