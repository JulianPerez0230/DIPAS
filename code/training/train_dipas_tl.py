...
Estructura de entrenamiento en 3 etapas:
Fase A: Pre-entrenamiento geométrico incondicional con 10.000 muestras de UniFoil (data/unifoil_cst_dataset.csv).
Fase B: Fine-tuning condicional con 10.000 muestras de XFOIL (data/dataset.csv).
Fase C: Fine-tuning/ajuste final con ~3.000 muestras de Fluent (data/dataset_cfd.csv).
"""
...
def main():
    ...
    model_save_path = output_dir / "dipas_tl_model.pth"
    loss_plot_path = output_dir / "dipas_tl_losses.png"
    recon_plot_path = output_dir / "dipas_tl_evaluation_extremes.png"
    
    if not all(p.exists() for p in (unifoil_path, xfoil_path, cfd_path)):
        print("❌ Faltan datasets. Asegúrate de tener unifoil_cst_dataset.csv, dataset.csv y dataset_cfd.csv.")
        sys.exit(1)
        
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f">> Iniciando Experimento 2 (Transfer Learning) en {device}...")
    
    # -------------------------------------------------------------
    # FASE A: Pre-entrenamiento geométrico con UniFoil (Incondicional)
    # -------------------------------------------------------------
    print("\n--- Fase A: Pre-entrenamiento Geométrico con UniFoil (10k) ---")
    batch_size = 64
    epochs_unifoil = 80
    lr_unifoil = 0.001
    ...
    # Inicializamos modelo condicional, pero en la fase A le pasamos condiciones vacías (cond_dim=4 para el modelo final)
    ...
    for epoch in range(1, epochs_unifoil + 1):
        ...
        for batch_x, _ in unifoil_loader:
            ...
            # En modo incondicional para preentrenamiento, creamos condiciones artificiales en cero
    ...
    # -------------------------------------------------------------
    # FASE B: Fine-tuning condicional con XFOIL (10k)
    # -------------------------------------------------------------
    print("\n--- Fase B: Fine-tuning Condicional con XFOIL (10k) ---")
    epochs_xfoil = 60
    lr_xfoil = 0.0005 # Tasa más baja para ajustar el espacio condicional
    ...
    # -------------------------------------------------------------
    # FASE C: Fine-tuning de alta fidelidad con Fluent (3k)
    # -------------------------------------------------------------
    print("\n--- Fase C: Fine-tuning final con Fluent ---")
    epochs_cfd = 60
    lr_cfd = 0.0001
    ...
    print(f"\n>> Modelo final DIPAS TL guardado en: {model_save_path}")
    
    # Graficar curva de pérdidas
    plt.figure(figsize=(8, 4))
    plt.plot(train_losses, label="Train Loss (CFD)")
    plt.plot(val_losses, '--', label="Val Loss (CFD)")
    plt.title("Curva de Pérdidas de Ajuste Fino CFD - DIPAS TL (Exp 2)")
    plt.xlabel("Épocas")
    plt.ylabel("Pérdida")
    plt.legend()
    plt.grid(True)
    ...
    print("==========================================================================")
    print("                      DIPAS TL (Exp 2): Metricas de Validacion            ")
    print("==========================================================================")
    ...
    for idx, (label, name) in enumerate(targets.items()):
        ...
        ax.plot(x_pts, y_up_rec, 'r--', label="DIPAS TL (IA)", linewidth=1.5)
    ...
    plt.close()
    print("\n>> Experimento 2 finalizado con éxito.")
